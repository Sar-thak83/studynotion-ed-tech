...
const UploadFile = ({
  ...
  const onDropRejected = (rejectedFiles) => {
    ...
    if (error.code === "file-too-large") {
      toast.error(
        `Please enter a ${video ? "video" : "image"} file of less than ${
          video ? "10 MB" : "200 KB"
        }`
      );
  ...
  return (
    <div className="flex flex-col">
      ...
      <div
        className={`flex justify-center items-center cursor-pointer rounded-md min-h-[250px] border-2 border-dotted border-[#585D69]
      ...
      >
        {previewSource ? (
          <div className="flex flex-col p-6">
            {video ? (
              <video
                ...
                src={previewSource}
                controls
                className="min-w-[250px] md:min-w-[350px] h-full w-full object-fill rounded-md"
              ></video>
            ) : (
              <img
                src={previewSource}
          ...
          <div {...getRootProps()} className="flex flex-col items-center p-6">
            ...
            <div className="grid place-items-center h-14 aspect-square rounded-full bg-[#171717]">
              <FiUploadCloud className="text-2xl text-[#FFD60A]" />
            </div>

            <p className="max-w-[200px] mt-2 text-center text-sm text-[#999DAA]">
              Drag and drop an {video ? "video" : "image"}, or click to{" "}
              <span className="font-semibold text-[#FFD60A]">Browse</span> file
            </p>

            <ul className="mt-10 flex justify-between gap-x-12 text-center text-xs text-[#999DAA] list-disc">
