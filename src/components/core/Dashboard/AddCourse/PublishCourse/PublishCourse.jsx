...
const PublishCourse = () => {
  ...
  const handleSaveChanges = async (data) => {
    const courseStatus = getValues("makePublic")
  ...
  return (
    <div className="bg-[#161D29] rounded-md p-6 border border-[#2C333F] space-y-6">
      ...
      <form onSubmit={handleSubmit(handleSaveChanges)}>
        <div className="flex items-center mb-8 gap-x-2">
          <input
            ...
            id="makePublic"
            disabled={loading}
            className="h-4 w-4 focus:ring-2 focus:ring-[#F1F2FF] rounded bg-[#585D69] text-[#6E727F]  border-[#888888]"
            {...register("makePublic")}
          />
          <label className="text-lg text-[#6E727F]" htmlFor="makePublic">
