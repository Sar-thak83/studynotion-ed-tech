...
const CourseTable = ({ courses, setCourses }) => {
  ...
  return (
    <div className="rounded-md border border-[#161D29] overflow-x-auto">
      <div className="w-[850px] md:w-full">
        <table>
          <thead>
            <tr className="flex gap-x-10 border-b border-b-[#161D29] py-2 px-6">
              <th className="flex-1 text-left text-sm uppercase font-medium text-[#AFB2BF]">
                Course
              </th>
              <th className="text-left text-sm uppercase font-medium text-[#AFB2BF]">
                Duration
              </th>
              <th className="text-left text-sm uppercase font-medium text-[#AFB2BF]">
                Price
          ...
          <tbody className="flex flex-col gap-y-5">
            {courses.map((course) => (
              <tr
                key={course._id}
                className="mt-0 flex gap-x-10 border-b border-b-[#161D29] py-8 px-6"
              >
                <td>
                  <Link
                    to={
                    ...
                    <div className="flex flex-1 gap-x-4 cursor-pointer">
                      ...
                      <div className="flex flex-col justify-between">
                        ...
                        <p className="text-xs text-white">
                          Created: {dateTimeFormatter(course.createdAt)}
                        </p>
                        <div>
                          {course.status === COURSE_STATUS.DRAFT ? (
                            <div className="flex gap-x-2 items-center w-fit text-sm py-[2px] px-2 rounded-full text-[#F37290] bg-[#2C333F] font-medium">
                              <HiClock size={14} />
                              <span>Drafted</span>
                            </div>
                          ) : (
                            <div className="flex gap-x-2 items-center w-fit text-sm py-[2px] px-2 rounded-full text-[#E7C009] bg-[#2C333F] font-medium">
                              <div className="grid place-items-center bg-[#E7C009] text-[#2C333F] h-3 aspect-square rounded-full">
                                <FaCheck size={8} />
                              </div>
                              <span>Published</span>
                            </div>
