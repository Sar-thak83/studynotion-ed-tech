...
const InstructorChart = ({ dashboardData }) => {
  ...
  const studentsPieData = {
    labels: dashboardData.coursesWithStats.map(
      (courseWithStats) => courseWithStats.course.title
    ),
    datasets: [
      {
        label: "# of Enrolled Students",
        data: dashboardData.coursesWithStats.map(
          (courseWithStats) => courseWithStats.stats.totalStudents
        ),
    ...
    datasets: [
      {
        ...
        data: dashboardData.coursesWithStats.map(
          (courseWithStats) => courseWithStats.stats.totalIncome
        ),
  ...
  useEffect(() => {
    setStudentsPieColors(
      generateRandomColors(dashboardData.totalPublishedCourses)
    );
    setIncomePieColors(
      generateRandomColors(dashboardData.totalPublishedCourses)
    );
  ...
  return (
    <div className="bg-[#161D29] h-full rounded-md p-6 flex flex-col flex-1 gap-y-4 ">
      <p className="text-lg text-[#F1F2FF] font-bold">Visualize</p>
