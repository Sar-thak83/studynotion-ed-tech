...
export const buyCourses = async (
  ...
  try {
    ...
    const options = {
      key: import.meta.env.VITE_RAZORPAY_PAY_KEY_ID,
    ...
    toast.error(
      error?.response?.data?.error || "Could not make payment ! Try again"
    );
  }
  toast.dismiss(toastId);
};
...
export const sendPaymentSuccess = async (response, token) => {
  ...
  try {
    await apiConnector(
      "POST",
      paymentApi.POST_SEND_PAYMENT_SUCCESS_EMAIL_API,
      response,
      {
        Authorization: `Bearer ${token}`,
      }
    ...
    toast.error(
      error?.response?.data?.error || "Failed to send Payment Success Email"
    );
  }
  toast.dismiss(toastId);
};
...
export const verifyPaymentSignature = async (
  ...
  try {
    ...
    toast.error(
      error?.response?.data?.error || "⚠️ Payment Verification Failed"
    );
  }
