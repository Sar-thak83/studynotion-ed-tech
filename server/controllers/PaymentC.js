  ...
  try {
    ...
    for (const courseId of courses) {
      ...
      if (course.studentsEnrolled.includes(user._id)) {
        return next(
          new ErrorResponse("Student is already enrolled in a course", 404)
        );
      }
...
exports.verifyPaymentSignature = async (req, res, next) => {
  try {
    ...
    const generated_signature = crypto
      .createHmac("sha256", process.env.RAZORPAY_PAY_KEY_SECRET)
...
const enrollStudent = async (courses, userId, res, next) => {
  if (!(courses && courses.length && userId)) {
    return next(
      new ErrorResponse("Please provide course and user details", 404)
    );
  }
...
exports.capturePayment = async (req, res, next) => {
  try {
    ...
    if (!course) {
      return next(
        new ErrorResponse(
          "Could not find the course, please enter vaild course details",
          404
        )
      );
    }
    ...
    try {
      ...
      return next(
        new ErrorResponse("Could not create order. Please try again", 500)
      );
    }
  } catch (err) {
    next(new ErrorResponse("Failed to create order. Please try again", 500));
...
exports.verifySignature = async (req, res, next) => {
  try {
    ...
    const shasum = crypto.createHmace(
      "sha256",
...
const addCourse = async (res, courseId, userId) => {
  ...
  try {
    const emailResponse = await emailSender(
      enrolledUser.email,
      "Congratulations for buying new course from StudyNotion",
      courseEnrollmentEmailTemplate(
